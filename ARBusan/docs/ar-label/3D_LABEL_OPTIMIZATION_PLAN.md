...
- 구현: WGS84 최종 표시 좌표가 Polygon 내부 또는 외곽선 허용 오차 0.8m 이내인지 검증한다.
- 구현: 5m 이내 근거리 내부 보정 좌표가 Polygon 허용 범위 밖이면 원래 외벽점으로 되돌린다.
- 구현: Polygon 기준 검증을 통과하지 못한 WGS84 후보는 생성하지 않는다.
- 구현: 기존 3D anchor가 있는 건물은 내 위치 이동만으로 `가장 가까운 외벽점`을 계속 재계산하지 않고, 최초 안정 외벽점을 유지한다.

테스트:
- 건물 정면, 측면, 모서리에서 구체가 건물 외곽을 벗어나지 않는가?
- heading이 틀어져도 구체가 도로/다른 건물 쪽으로 밀려나지 않는가?
- 건물 정면을 유지한 채 좌우로 이동해도 구체가 사용자를 따라 외벽 밖으로 밀려오지 않는가?
...
5. 4.6 외벽 전환 hysteresis
6. 4.7 이동 smoothing과 최대 이동 속도 제한
