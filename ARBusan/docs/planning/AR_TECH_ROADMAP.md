...
사용자는 일반 2D 지도에서 부산 관광지/랜드마크를 둘러보고, 선택한 목적지는 필요할 때만 카메라를 켜서 AR 길찾기로 안내받는다.

- 둘러보기 모드: 앱 메인 화면은 2D 지도이며, 현재 위치 기준 1km 이내 TourAPI 관광지를 핀으로 표시한다.
- 관광지 선택: 지도 핀을 누르면 사진, 정보보기, 길찾기 버튼이 있는 라벨/카드를 표시한다.
- 정보보기: 1차 MVP에서는 새 창 전환만 구현하고 상세 콘텐츠는 후속으로 둔다.
- 길찾기 모드: 길찾기 버튼을 눌렀을 때만 카메라를 켜고, 선택한 목적지 1개를 TMAP 경로 기준으로 안내한다.
- 먼 관광지: 1km 범위 밖은 기본 후보에서 제외하거나 제한적으로만 안내한다.
...
길찾기:
    ...
    목적지 방향 2D 라벨과 turn boundary 3D 대형 화살표로 진행 방향 안내

둘러보기:
    일반 2D 지도
    현재 위치 기준 1km 이내 TourAPI 관광지 핀 표시
    핀 선택 시 사진/정보보기/길찾기 카드 표시

정보 표시:
    정보보기 버튼은 새 창 전환
    길찾기 중 목적지 방향은 2D 라벨/edge marker 중심
    도착 근처는 3D 대형 핀 표시
```

기존 외벽 부착형 3D 라벨과 VWorld 대표/정문 좌표 방식은 테스트 교훈만 남긴다. 앞으로 기본 탐색은 2D 지도이며, AR은 길찾기 버튼을 눌렀을 때만 켠다. 길찾기 모드에서는 선택 목적지 1개만 안내하고, 3D 요소는 회전 안내/도착 강조에 제한한다.
...
길찾기 모드:
    ...
    45도 이상 꺾임 지점 바운더리 안에서 전방 3D 대형 화살표 표시
    도착 5~15m 이내에서 3D 대형 핀 표시 후 길안내 종료

edge marker:
    길찾기 중 목적지가 화면 밖일 때 방향 안내
...
-> 응답 경로의 마지막 도착 좌표 추출
-> 길찾기 중에는 목적지 방향 2D 라벨/edge marker 표시
-> 45도 이상 꺾임 지점 바운더리 안에서 사용자 전방 3D 대형 화살표 표시
-> 도착 5~15m 이내에서 3D 대형 핀 표시
```
...
1. 모드 분리 UX 정리
   - 둘러보기: 2D 지도 + TourAPI 1km 관광지 핀 + 관광지 카드
   - 정보보기: 새 창 전환
   - 길찾기: 선택 목적지 1개 + AR 카메라 + 2D 방향 라벨 + turn boundary 3D 화살표
   - 주변 후보를 모두 AR에 띄우는 방향은 실험/디버그로 낮춘다.

2. TMAP 도착 좌표 방식으로 길찾기 3D 좌표 교체
   - 방향 변경: 대표/정문 자동 계산이 아니라 TMAP 보행자 마지막 도착 좌표를 기본으로 한다.
   - 기존 외벽 후보점/Polygon 대표점은 길찾기 MVP에서 삭제한다.
   - TMAP 키 로딩, 클라이언트, 경로 마지막 좌표 파싱, 캐시를 추가한다.

3. 3D 마커와 2D 지도/라벨 역할 분리
   - 둘러보기의 이름/거리/선택은 2D 지도와 카드가 담당한다.
   - 길찾기 중 진행 방향은 2D 라벨/edge marker가 담당한다.
   - 3D는 회전 안내와 도착 강조만 담당한다.
...
5. TourAPI 재연결
   - 목업에서 검증한 `POI -> TMAP 도착점 -> 2D 방향 라벨/turn boundary 3D 화살표/도착 핀` 흐름을 TourAPI 후보에 적용한다.
...
상세 기준은 `../ar-label/3D_LABEL_OPTIMIZATION_PLAN.md`를 따른다.

1. 3D 구체/마커
   - Polygon 외벽점 기반 배치를 삭제한다.
   - 선택 목적지뿐 아니라 둘러보기 구체도 TMAP 마지막 도착 좌표에 고정한다.
   - 사용자가 움직여도 구체가 따라오면 실패다.

2. 길찾기 화살표
   - TMAP 경로에서 직진 대비 45도 이상 꺾이는 지점을 계산한다.
   - 사용자가 turn point 바운더리 안에 들어오면 사용자 전방 2~4m, 기기 높이 근처에 대형 화살표를 띄운다.
   - 화살표를 turn point 좌표나 바닥에 정밀 고정하지 않는다.
   - turn point를 지나거나 방향이 정렬되면 화살표를 제거한다.
