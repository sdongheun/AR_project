...
기존 외벽 부착형 3D 라벨과 VWorld 대표/정문 좌표 방식은 테스트 교훈만 남긴다. 앞으로 3D 마커 좌표는 둘러보기/길찾기 모두 TMAP 마지막 도착 좌표에 고정한다. 길찾기 모드에서는 선택 목적지 1개만 강조하고, 둘러보기는 반원형 2D 레이더와 가벼운 카메라 마커를 중심으로 한다.
...
2. 길찾기 화살표
   - TMAP 경로를 따라 지면 위에 보여야 한다.
   - 로그상 생성됐더라도 화면에서 안 보이면 아직 실패다.
   - 바닥 아래에 박히거나 공중에 떠 있지 않도록 별도 검증한다.
