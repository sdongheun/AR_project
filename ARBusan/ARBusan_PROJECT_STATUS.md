...
- 프로젝트: `ARBusan`
- 방식: iOS Swift 네이티브
- 현재 MVP 방향: **AR 관광 체험** — ① 카메라 관광지 인식(2D 태그·발견) ② **도착 AR 포토존 + 스탬프**(근거리 3D, AR 정체성). 길찾기는 **보조**(2D 지도 주력 + 목적지 표시). 상세: `docs/planning/AR_MVP_DIRECTION.md`, `docs/experience/PHOTO_ZONE_STAMP_MVP.md`
- 길찾기 AR 정리: **회전 화살표·곡선 리본·출발 방향·턴바이턴 제거 완료**. 남은 길찾기 AR = 목적지 표시(원거리 2D 라벨/가장자리, 근거리 3D 빨간 핀) + 견고성 §4-A/B/C.
- 다음 작업: **도착 AR 포토존 스파이크**(핀 탭 → 3D 타이포 + 핀치/회전 + 캡처 + 스탬프)
...
- 도착 판정/도착 핀(체크리스트 3.8) 구현: 도착 좌표 10m 이내 진입 시 길안내 종료 + 목적지 2D 라벨/edge marker 숨김 + 3D 대형 도착 핀 표시(`ArrivalPinSnapshot` → `ARSessionViewController.setArrivalPin`). 핀은 카메라 시선을 따라가지 않고 **기기 위치+높이, 목적지 방위 전방, 중력 정렬**로 배치한다(`ARBusan/Navigation/TravelDirectionAnchor.swift`, 3.10 앵커링 규칙). 실기기에서 초록 도착 핀 표시 확인 완료.
- 바닥 리본 + 주행 방향 앵커링(체크리스트 3.10) 구현: 회전 chevron을 카메라 시선 추종에서 **주행 방향 앵커(기기 위치+높이, 중력 정렬)**로 교체하고, 가는 방향으로 뻗는 **바닥 리본**(`RouteRibbonSnapshot`)을 추가. 길찾기 중 상단 2D 방향 라벨/edge marker는 숨기고(하단 2D 지도가 경로 담당), 도착/불안정일 때만 텍스트 배너 표시. 위치/heading 불안정 시 리본·화살표를 숨긴다. 곡선 리본(경로 폴리라인 따라 휨)과 실기기 시각 검증·튜닝은 후속.
...
- 거리감 1차(TURN_UX_RULES_V2 §2): 회전 화살표를 실제 회전 거리에 비례 배치(2.5~12m 클램프)하고, `navigationTurnBanner`("Nm 후 좌/우회전")를 회전 활성 시 상단에 표시. 경로 누적 거리 정밀화·횡단보도/계단 특수 안내는 후속.
- 견고성 §4-A(경로 이탈 재탐색): 자동(경로선 40m 초과 8초 지속+쿨다운 15초, `NavigationReroute.shouldAutoReroute`)과 수동(하단 2D 지도 우상단 **"다시 맞추기" 스마트 버튼**) 병행. 수동은 항상 위치/heading 보정(무료)하고 **실제 이탈(20m 초과)/경로 없음일 때만 경로 재탐색(API)** → 온-루트에서 누른 불필요한 호출 방지. 새 경로 도착 전까지 기존 안내 유지(soft, `forceReroute`).
