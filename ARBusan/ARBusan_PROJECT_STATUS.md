...
- 프로젝트: `ARBusan`
- 방식: iOS Swift 네이티브
- 현재 MVP 방향: **둘러보기 반원형 2D 레이더 + 길찾기 TMAP 도착점 3D 마커/화살표 안내**
...
둘러보기 모드:
    카메라 + 하단 반원형 2D 레이더
    레이더에 내 시야 방향과 주변 관광지/건물 마커 표시
    카메라에는 선택 후보 또는 화면 안 후보만 가볍게 표시

길찾기 모드:
    선택한 목적지 1개만 TMAP 도착 좌표 기반 3D 마커로 표시
...
기존 “건물 외벽에 3D 텍스트를 직접 붙이는 방식”과 “VWorld Polygon 기반 대표/정문 좌표 자동 계산”은 테스트 교훈만 남긴다. 새 기본 방향은 둘러보기에서는 레이더 중심, 길찾기에서는 TMAP 보행자 길찾기의 마지막 도착 좌표에 3D 마커를 고정하고 경로 화살표로 안내하는 방식이다.
...
- 앞으로는 외벽점을 계속 따라가는 방식이 아니라 둘러보기는 반원형 2D 레이더, 길찾기는 TMAP 보행자 길찾기 마지막 도착 좌표를 3D 마커 기준으로 사용한다.
...
1. 둘러보기/길찾기 모드 분리
   - 둘러보기: 하단 반원형 2D 레이더 + 카메라 보조 마커
   - 길찾기: 선택 목적지 1개 + TMAP 도착 좌표 3D 마커 + 경로 화살표
2. TMAP 도착 좌표 방식으로 길찾기 3D 기본 좌표 교체
   - 방향 변경: TMAP 보행자 경로 마지막 도착 좌표를 길찾기 3D 기본 좌표로 사용
   - 다음: TMAP 도착 좌표 캐시/로그/실외 검증
