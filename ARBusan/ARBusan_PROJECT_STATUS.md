...
- 프로젝트: `ARBusan`
- 방식: iOS Swift 네이티브
- 현재 MVP 방향: **하단 반원형 지도/레이더 + 카메라 AR 방향 안내**
...
```text
둘러보기 모드:
    카메라 + 하단 반원형 2D 지도/레이더
    레이더에 내 시야 방향과 주변 관광지/건물 마커 표시
    카메라에는 큰 방향 안내판/화살표를 표시하는 방향으로 전환

길찾기 모드:
    선택 목적지 1개를 TMAP 경로 기준으로 안내

길찾기 안내:
    정밀 바닥 부착 화살표보다 화면상 큰 방향 안내를 우선

도착 근처:
    TMAP 마지막 도착 좌표 기준 목적지 핀/도착 안내
```

기존 “건물 외벽에 3D 텍스트를 직접 붙이는 방식”, “VWorld Polygon 기반 대표/정문 좌표 자동 계산”, “정밀 바닥 화살표”는 테스트 교훈만 남기고 기본 MVP에서 제외한다. 새 기본 방향은 하단 지도/레이더로 안정적인 기준점을 제공하고, 카메라 화면에는 읽기 쉬운 큰 AR 방향 안내를 표시하는 방식이다.
...
- 앞으로는 외벽점을 계속 따라가는 방식이 아니라 하단 반원형 지도/레이더와 카메라 AR 방향 안내를 메인으로 사용한다.
...
1. 하단 반원형 지도/레이더를 MVP 메인 UI로 정리
2. TMAP 경로 기반 다음 행동 안내 문구 계산
3. 카메라 화면 위 큰 AR 방향 안내판/화살표 구현
4. 도착 근처 목적지 핀 표시 방식 재정의
5. TourAPI 부산 후보 재연결
